...
func (mp Mailpit) SendConfirmTripEmailToTripOwner(tripID uuid.UUID) error {
	...
	client, err := mail.NewClient(
		...
		mail.WithPort(1025),
			...
			return fmt.Errorf("mailpit: failed to set email client in SendConfirmTripEmailToTripOwner: %w", err)
		}

		if err := client.DialAndSend(msg); err != err {
			return fmt.Errorf("mailpit: failed to send email in SendConfirmTripEmailToTripOwner: %w", err)
		}
